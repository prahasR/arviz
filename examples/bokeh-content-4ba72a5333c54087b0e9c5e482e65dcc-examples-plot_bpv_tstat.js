(function() {
  const fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      const element = document.getElementById("48a12ec4-cbe6-47d3-801e-dce6accebfb7");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '48a12ec4-cbe6-47d3-801e-dce6accebfb7' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.2.min.js"];
      const css_urls = [];
      
    
      const inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  const docs_json = '{"19f8d2da-f286-407c-b531-b7f99bafffff":{"defs":[],"roots":{"references":[{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17918","type":"Line"},{"attributes":{},"id":"17926","type":"BasicTickFormatter"},{"attributes":{},"id":"17958","type":"UnionRenderers"},{"attributes":{},"id":"17927","type":"AllLabels"},{"attributes":{"source":{"id":"17911"}},"id":"17916","type":"CDSView"},{"attributes":{"children":[{"id":"17965"},{"id":"17963"}]},"id":"17966","type":"Column"},{"attributes":{"coordinates":null,"data_source":{"id":"17917"},"glyph":{"id":"17918"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17920"},"nonselection_glyph":{"id":"17919"},"view":{"id":"17922"}},"id":"17921","type":"GlyphRenderer"},{"attributes":{"line_alpha":0.2,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17920","type":"Line"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17919","type":"Line"},{"attributes":{},"id":"17897","type":"UndoTool"},{"attributes":{"source":{"id":"17917"}},"id":"17922","type":"CDSView"},{"attributes":{"coordinates":null,"formatter":{"id":"17929"},"group":null,"major_label_policy":{"id":"17930"},"ticker":{"id":"17885"}},"id":"17884","type":"LinearAxis"},{"attributes":{},"id":"17932","type":"Selection"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"17932"},"selection_policy":{"id":"17931"}},"id":"17911","type":"ColumnDataSource"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17912","type":"Line"},{"attributes":{"children":[[{"id":"17875"},0,0]]},"id":"17963","type":"GridBox"},{"attributes":{},"id":"17959","type":"Selection"},{"attributes":{"fill_alpha":{"value":0.2},"fill_color":{"value":"#2a2eec"},"hatch_alpha":{"value":0.2},"line_alpha":{"value":0.2},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"17942","type":"Circle"},{"attributes":{"source":{"id":"17939"}},"id":"17944","type":"CDSView"},{"attributes":{"coordinates":null,"group":null,"text":"y / y","text_font_size":"15pt"},"id":"17945","type":"Title"},{"attributes":{},"id":"17934","type":"Selection"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"17921"}]},"id":"17938","type":"LegendItem"},{"attributes":{"data":{},"selected":{"id":"17959"},"selection_policy":{"id":"17958"}},"id":"17939","type":"ColumnDataSource"},{"attributes":{"callback":null},"id":"17899","type":"HoverTool"},{"attributes":{},"id":"17895","type":"WheelZoomTool"},{"attributes":{"coordinates":null,"data_source":{"id":"17911"},"glyph":{"id":"17912"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17914"},"nonselection_glyph":{"id":"17913"},"view":{"id":"17916"}},"id":"17915","type":"GlyphRenderer"},{"attributes":{},"id":"17882","type":"LinearScale"},{"attributes":{},"id":"17876","type":"DataRange1d"},{"attributes":{},"id":"17878","type":"DataRange1d"},{"attributes":{},"id":"17930","type":"AllLabels"},{"attributes":{"coordinates":null,"data_source":{"id":"17939"},"glyph":{"id":"17940"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17942"},"nonselection_glyph":{"id":"17941"},"view":{"id":"17944"}},"id":"17943","type":"GlyphRenderer"},{"attributes":{},"id":"17929","type":"BasicTickFormatter"},{"attributes":{"overlay":{"id":"17901"}},"id":"17896","type":"LassoSelectTool"},{"attributes":{"coordinates":null,"group":null,"items":[{"id":"17938"}]},"id":"17937","type":"Legend"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"17901","type":"PolyAnnotation"},{"attributes":{},"id":"17931","type":"UnionRenderers"},{"attributes":{"overlay":{"id":"17900"}},"id":"17894","type":"BoxZoomTool"},{"attributes":{"coordinates":null,"formatter":{"id":"17926"},"group":null,"major_label_policy":{"id":"17927"},"ticker":{"id":"17889"}},"id":"17888","type":"LinearAxis"},{"attributes":{"axis":{"id":"17888"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"17891","type":"Grid"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"17900","type":"BoxAnnotation"},{"attributes":{"data":{},"selected":{"id":"17934"},"selection_policy":{"id":"17933"}},"id":"17917","type":"ColumnDataSource"},{"attributes":{},"id":"17893","type":"PanTool"},{"attributes":{},"id":"17880","type":"LinearScale"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17913","type":"Line"},{"attributes":{},"id":"17885","type":"BasicTicker"},{"attributes":{"line_alpha":0.2,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17914","type":"Line"},{"attributes":{},"id":"17898","type":"SaveTool"},{"attributes":{"toolbars":[{"id":"17902"}],"tools":[{"id":"17892"},{"id":"17893"},{"id":"17894"},{"id":"17895"},{"id":"17896"},{"id":"17897"},{"id":"17898"},{"id":"17899"}]},"id":"17964","type":"ProxyToolbar"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"hatch_alpha":{"value":0.1},"line_alpha":{"value":0.1},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"17941","type":"Circle"},{"attributes":{},"id":"17889","type":"BasicTicker"},{"attributes":{},"id":"17933","type":"UnionRenderers"},{"attributes":{"axis":{"id":"17884"},"coordinates":null,"group":null,"ticker":null},"id":"17887","type":"Grid"},{"attributes":{"tools":[{"id":"17892"},{"id":"17893"},{"id":"17894"},{"id":"17895"},{"id":"17896"},{"id":"17897"},{"id":"17898"},{"id":"17899"}]},"id":"17902","type":"Toolbar"},{"attributes":{},"id":"17892","type":"ResetTool"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"17940","type":"Circle"},{"attributes":{"below":[{"id":"17884"}],"center":[{"id":"17887"},{"id":"17891"},{"id":"17937"}],"height":500,"left":[{"id":"17888"}],"output_backend":"webgl","renderers":[{"id":"17915"},{"id":"17921"},{"id":"17943"}],"title":{"id":"17945"},"toolbar":{"id":"17902"},"toolbar_location":null,"width":500,"x_range":{"id":"17876"},"x_scale":{"id":"17880"},"y_range":{"id":"17878"},"y_scale":{"id":"17882"}},"id":"17875","subtype":"Figure","type":"Plot"},{"attributes":{"toolbar":{"id":"17964"},"toolbar_location":"above"},"id":"17965","type":"ToolbarBox"}],"root_ids":["17966"]},"title":"Bokeh Application","version":"2.4.2"}}';
                  const render_items = [{"docid":"19f8d2da-f286-407c-b531-b7f99bafffff","root_ids":["17966"],"roots":{"17966":"48a12ec4-cbe6-47d3-801e-dce6accebfb7"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();