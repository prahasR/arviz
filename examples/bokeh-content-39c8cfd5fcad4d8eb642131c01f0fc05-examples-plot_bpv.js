(function() {
  const fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      const element = document.getElementById("9c3daf61-22a9-4cea-acb8-98b8e016a2ed");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '9c3daf61-22a9-4cea-acb8-98b8e016a2ed' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.2.min.js"];
      const css_urls = [];
      
    
      const inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  const docs_json = '{"e3f1d0fa-c536-465c-992c-a38afb7d7457":{"defs":[],"roots":{"references":[{"attributes":{},"id":"17761","type":"SaveTool"},{"attributes":{"axis":{"id":"17747"},"coordinates":null,"group":null,"ticker":null},"id":"17750","type":"Grid"},{"attributes":{"coordinates":null,"formatter":{"id":"17800"},"group":null,"major_label_policy":{"id":"17801"},"ticker":{"id":"17748"}},"id":"17747","type":"LinearAxis"},{"attributes":{},"id":"17760","type":"UndoTool"},{"attributes":{"data":{"x":[0,1],"y":[1,1]},"selected":{"id":"17805"},"selection_policy":{"id":"17804"}},"id":"17781","type":"ColumnDataSource"},{"attributes":{},"id":"17748","type":"BasicTicker"},{"attributes":{},"id":"17758","type":"WheelZoomTool"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17775","type":"Line"},{"attributes":{"line_alpha":0.2,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17790","type":"Line"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"17803"},"selection_policy":{"id":"17802"}},"id":"17774","type":"ColumnDataSource"},{"attributes":{},"id":"17805","type":"Selection"},{"attributes":{},"id":"17806","type":"UnionRenderers"},{"attributes":{},"id":"17752","type":"BasicTicker"},{"attributes":{},"id":"17739","type":"DataRange1d"},{"attributes":{},"id":"17741","type":"DataRange1d"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17776","type":"Line"},{"attributes":{"coordinates":null,"data_source":{"id":"17774"},"glyph":{"id":"17775"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17777"},"nonselection_glyph":{"id":"17776"},"view":{"id":"17779"}},"id":"17778","type":"GlyphRenderer"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"17763","type":"BoxAnnotation"},{"attributes":{"coordinates":null,"formatter":{"id":"17797"},"group":null,"major_label_policy":{"id":"17798"},"ticker":{"id":"17752"}},"id":"17751","type":"LinearAxis"},{"attributes":{},"id":"17802","type":"UnionRenderers"},{"attributes":{},"id":"17807","type":"Selection"},{"attributes":{"toolbars":[{"id":"17765"}],"tools":[{"id":"17755"},{"id":"17756"},{"id":"17757"},{"id":"17758"},{"id":"17759"},{"id":"17760"},{"id":"17761"},{"id":"17762"}]},"id":"17812","type":"ProxyToolbar"},{"attributes":{"coordinates":null,"data_source":{"id":"17781"},"glyph":{"id":"17782"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17784"},"nonselection_glyph":{"id":"17783"},"view":{"id":"17786"}},"id":"17785","type":"GlyphRenderer"},{"attributes":{"toolbar":{"id":"17812"},"toolbar_location":"above"},"id":"17813","type":"ToolbarBox"},{"attributes":{"tools":[{"id":"17755"},{"id":"17756"},{"id":"17757"},{"id":"17758"},{"id":"17759"},{"id":"17760"},{"id":"17761"},{"id":"17762"}]},"id":"17765","type":"Toolbar"},{"attributes":{},"id":"17798","type":"AllLabels"},{"attributes":{},"id":"17755","type":"ResetTool"},{"attributes":{},"id":"17797","type":"BasicTickFormatter"},{"attributes":{"children":[[{"id":"17738"},0,0]]},"id":"17811","type":"GridBox"},{"attributes":{},"id":"17804","type":"UnionRenderers"},{"attributes":{},"id":"17756","type":"PanTool"},{"attributes":{},"id":"17745","type":"LinearScale"},{"attributes":{},"id":"17800","type":"BasicTickFormatter"},{"attributes":{},"id":"17803","type":"Selection"},{"attributes":{"children":[{"id":"17813"},{"id":"17811"}]},"id":"17814","type":"Column"},{"attributes":{"overlay":{"id":"17764"}},"id":"17759","type":"LassoSelectTool"},{"attributes":{},"id":"17801","type":"AllLabels"},{"attributes":{"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"17782","type":"Line"},{"attributes":{"source":{"id":"17774"}},"id":"17779","type":"CDSView"},{"attributes":{"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17788","type":"Line"},{"attributes":{"line_alpha":0.2,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17777","type":"Line"},{"attributes":{"below":[{"id":"17747"}],"center":[{"id":"17750"},{"id":"17754"},{"id":"17780"}],"height":500,"left":[{"id":"17751"}],"output_backend":"webgl","renderers":[{"id":"17778"},{"id":"17785"},{"id":"17791"}],"title":{"id":"17793"},"toolbar":{"id":"17765"},"toolbar_location":null,"width":500,"x_range":{"id":"17739"},"x_scale":{"id":"17743"},"y_range":{"id":"17741"},"y_scale":{"id":"17745"}},"id":"17738","subtype":"Figure","type":"Plot"},{"attributes":{"coordinates":null,"group":null,"text":"y / y","text_font_size":"15pt"},"id":"17793","type":"Title"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17789","type":"Line"},{"attributes":{"overlay":{"id":"17763"}},"id":"17757","type":"BoxZoomTool"},{"attributes":{"coordinates":null,"data_source":{"id":"17787"},"glyph":{"id":"17788"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17790"},"nonselection_glyph":{"id":"17789"},"view":{"id":"17792"}},"id":"17791","type":"GlyphRenderer"},{"attributes":{"source":{"id":"17781"}},"id":"17786","type":"CDSView"},{"attributes":{"callback":null},"id":"17762","type":"HoverTool"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"17764","type":"PolyAnnotation"},{"attributes":{"data":{},"selected":{"id":"17807"},"selection_policy":{"id":"17806"}},"id":"17787","type":"ColumnDataSource"},{"attributes":{"source":{"id":"17787"}},"id":"17792","type":"CDSView"},{"attributes":{"bottom":1.459677659268402,"coordinates":null,"fill_alpha":0.1,"fill_color":"#2a2eec","group":null,"top":0.6850827603275131},"id":"17780","type":"BoxAnnotation"},{"attributes":{"line_alpha":0.1,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"17783","type":"Line"},{"attributes":{"line_alpha":0.2,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"17784","type":"Line"},{"attributes":{"axis":{"id":"17751"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"17754","type":"Grid"},{"attributes":{},"id":"17743","type":"LinearScale"}],"root_ids":["17814"]},"title":"Bokeh Application","version":"2.4.2"}}';
                  const render_items = [{"docid":"e3f1d0fa-c536-465c-992c-a38afb7d7457","root_ids":["17814"],"roots":{"17814":"9c3daf61-22a9-4cea-acb8-98b8e016a2ed"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();